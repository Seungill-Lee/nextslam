...
export default function GbWrite(props) {
    ...
    useEffect(() => {
        if(gb && mode == "PATCH") {
            ...
            setContentLen(gb.content.length - (gb.content.split('\n').length -1));
        }
    },[])
            ...
            <fieldset>
                ...
                <dl>
                    ...
                    <div className={`${scss.field} ${scss.content}`}>
                        ...
                        <dd><textarea cols="30" rows="10" minLength="30" maxLength="1000" name="content" placeholder={`내용을 입력해주세요. \n[ 최소 30자, 최대 1000자 ]`} id={`${Labeling}content`} value={gbContent ? gbContent : ""} onChange={(e) => {{setGbContent(e.target.value)} {setContentLen(e.target.value.length)}}} ref={contArea} required></textarea></dd>
                    </div>
                </dl>
